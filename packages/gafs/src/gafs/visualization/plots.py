...
``matplotlib`` is an optional dependency (install with ``pip install gafs[viz]``)
...
from gafs.core.config import SelectionResult
...
def _require_matplotlib():
    try:
        ...
        raise ImportError(
            "matplotlib is required for plotting. Install it with `pip install gafs[viz]`."
        ) from exc
    return plt
